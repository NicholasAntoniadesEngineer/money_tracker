/**
 * .s6_budget_migration_runner.cjs  (UNTRACKED, money_tracker-owned, throwaway runner)
 *
 * Quadruplet of .s10_budget_runner.cjs (S1), .s2_budget_key_runner.cjs (S2), and
 * .s4s5_budget_transform_runner.cjs (S4/S5), pointed at the S6 one-time budget
 * migration gates (verify-before-destroy, idempotent, batched) over the REAL
 * BudgetMigrationService + BudgetCryptoService + DatabaseService dual-read
 * transforms, with a mocked row store + DEK.
 *
...
// nacl vendor-dir resolution (same rationale as the S1/S2/S4S5 runners): _harness.js
...
const testPath = path.resolve(__dirname, 'shared/services/budgetMigration.s6.test.js');
loadCjs(testPath);
