...
const MonthlyBudgetController = {
    ...
    addFixedCostRow(costData = null) {
        ...
        if (deleteBtn) {
            ...
            this.updateCalculations();
                // Repopulate working section when fixed cost is deleted (force update)
        ...
        row.querySelectorAll('input').forEach(input => {
            input.addEventListener('input', () => {
                ...
                this.updateCalculations();
                // Repopulate working section when fixed cost changes (force update)
                this.populateWorkingSectionFromCosts(true);
            });
    ...
    updateVariableCostTotal(textarea) {
        ...
        if (inlineAdjustmentMatch) {
            ...
            // Parse all inline adjustments as deductions (ignore the base estimate)
            // e.g., "40-50-10" -> -50 + -10 = -60 (absolute value = 60)
            const adjustmentMatches = adjustmentsStr.match(/[+\-][\d.]+/g);
            if (adjustmentMatches) {
                adjustmentMatches.forEach(match => {
                    if (match.startsWith('+')) {
            ...
            // First line is just a number (no adjustments)
            deductionsTotal = 0;
        }
        ...
        // Calculate total: sum of all deductions (positive values only)
        const total = deductionsTotal;
