...
const EncryptionFacade = {
    ...
    async initialize(config, userId) {
        ...
        const result = await KeyManagementService.initialize(config, userId);
        ...
        if (result.needsRestore) {
            ...
            return { success: false, needsRestore: true, keyMismatch: result.keyMismatch };
        }
    ...
    async decryptMessage(conversationId, encryptedData, senderId) {
        ...
        return await KeyManagementService.decryptMessage(conversationId, encryptedData, senderId);
    },
